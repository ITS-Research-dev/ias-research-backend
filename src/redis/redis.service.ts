...
private initializeRedis() {
    ...
    // 1. Parsing port ke number secara eksplisit
    const portRaw = this.configService.get<string | number>('REDIS_PORT', 6379);
    const port = typeof portRaw === 'string' ? parseInt(portRaw, 10) : portRaw;

    // 2. Tangani password agar hanya bernilai string atau undefined
    const password = this.configService.get<string>('REDIS_PASSWORD') || undefined;

    // 3. Parsing DB ke number secara eksplisit
    const dbRaw = this.configService.get<string | number>('REDIS_DB', 0);
    const db = typeof dbRaw === 'string' ? parseInt(dbRaw, 10) : dbRaw;
